...
requires less resources to run.  Likewise, since the container is not running the entirety of a complete operating system,
...
Here is a list of the fundamental podman commands and their purpose:
  ...
  * *podman pull* - pulls (copies) container image from repository (e.g. registry.redhat.io, hub.docker.com, etc.)
...
Container images can also be tagged with convenient, custom names.  This could make it more intuitive to understand what they 
